...
public class Grid {
    ...
    public int add(int rawCol, Player player) {
        ...
        while (i < lineNumber+1) {
            if (map[lineNumber-i][rawCol-1] == '.') {
                map[lineNumber-i][rawCol-1] = player.getX();
                break;
            }
            if (map[lineNumber-i][rawCol-1] == '.') {
                map[lineNumber-i][rawCol-1] = player.getX();
    ...
    /* FuncVictory a pour principe de prendre une partie de la grille, est comparé chaque point de cette partie avec N elements en dessous (verticale)
    ...
    sign: qui est 'X' ou '0'
    beginx: Le début de la partie de la grille ou l'on va commencer selon x
    beginy: le début de la partie de la grille ou l'on va commencer selon y
